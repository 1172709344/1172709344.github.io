...
```text
release/*  -> 必须 SemVer，禁止覆盖，可进入 Prod
sha-*      -> 自动生成的最简格式，强调源码追踪，可进入 Dev/Test
ci-*       -> 自动生成的组合格式，兼顾构建时间、run 与源码追踪，可进入 Dev/Test
channel/*  -> 可变别名，仅用于发现，不作为部署和审计主键
```

`sha-*` 与 `ci-*` 是同一自动生成类别下的两种命名策略，不需要在同一条流水线中先后兜底。团队选择一种作为默认规范即可；无论选择哪一种，都应设置保留期，且不得绕过发布门禁直接进入 Prod。
